...
public class UcManageAccessCodeImpl extends AbstractAccessCodeUc implements UcManageAccessCode {
  ...
  public boolean deleteAccessCode(long accessCodeId) {
    ...
    /**
     * Using the method getQueuemanagement() gives access to the methods that were created earlier in the
     * usecasemanage(inside the queue component). This is done so each component takes care of its own modifications.
     */
  ...
  public AccessCodeEto saveAccessCode(AccessCodeEto accessCodeEto) {
     ...
     * call then implemention of the method inside (UcFindAccessCodeImpl) through the interface. This allows us to use
